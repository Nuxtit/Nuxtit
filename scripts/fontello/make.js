...
const notGlyphs = ['fa-btn', 'fa-fw', 'fa-spin'];

const scannedIconList = shell_exec(
  'egrep -R "fa-[a-z-]{1,}" ' +
    scanfilesat +
    ' --only-matching --no-filename | sort | uniq',
)
  .split('\n')
  .filter(Boolean)
...
glyphs.forEach(data => {
  ...
  if (fcGlyphs.length !== 1) {
    ...
    console.log(
      'expected 1 glyph result, ',
      fcGlyphs.length,
      ' found for ',
      glyphName,
      ', is probably an alias, check http://fontello.com/',
    );
    process.exit(1);
  }
});
...
console.log(
  shell_exec(
    'npx fontello-cli install --config ./fontello.json --css ' +
      css +
      ' --font ' +
      font,
  ),
...
fs.writeFileSync(
  ...
  fs
    .readFileSync('./static/fa/css/fontello.css')
    .toString()
